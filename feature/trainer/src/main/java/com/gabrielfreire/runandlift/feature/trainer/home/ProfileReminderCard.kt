...
/**
 * O aviso de perfil incompleto, na home do treinador.
 *
 * É a segunda chance do que o passo a passo deixou pular — e por isso **avisa, não bloqueia**. Nada
 * do que falta impede alguém de prescrever para quem já é aluno dele, e um bloqueio aqui puniria
 * justamente quem preferiu começar antes de responder tudo.
 *
 * Usa o papel `attention` do tema, e não `critical`: perfil incompleto não é falha nem erro, é uma
 * pendência. Pintá-lo de vermelho ensinaria a ignorar o vermelho que importa.
 *
 * O texto diz **quantas** perguntas faltam, e a linha de baixo diz o que se ganha respondendo — que
 * aqui é ser encontrado por quem procura treinador, e não um treino melhor.
 *
...
internal fun ProfileReminderCard(missingCount: Int, onClick: () -> Unit, modifier: Modifier = Modifier) {
    ...
    Surface(
        ...
        Column(
            ...
            Text(
                text = pluralStringResource(R.plurals.trainer_home_reminder_title, missingCount, missingCount),
                style = MaterialTheme.typography.titleSmall,
            )
            Text(
                text = stringResource(R.string.trainer_home_reminder_body),
