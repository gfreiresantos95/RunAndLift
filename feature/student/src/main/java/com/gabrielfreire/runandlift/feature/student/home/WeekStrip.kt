 ...
 *
 * **Três canais para três estados, e nenhum deles é só a cor.** Cumprido leva o visto, previsto
 * leva o contorno vazio, descanso não leva nada — o mesmo par de sinais dos chips de escolha, pela
 * mesma razão (E0-09).
 *
...
/** A marca do dia: preenchida com visto, vazia com contorno, ou apagada. */
@Composable
private fun DayBadge(state: TrainingDayState) {
    Surface(
        ...
        Box(contentAlignment = Alignment.Center) {
            if (state == TrainingDayState.DONE) {
                Icon(
