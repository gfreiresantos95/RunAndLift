 ...
 *
 * Peso, altura e lesões só contam como faltando **se houver consentimento de dado de saúde**. Sem
 * ele, o app não pediu esses dados e não os pode guardar — cobrá-los no aviso seria cobrar uma
 * resposta que a pessoa já recusou dar, o que é o oposto de consentimento.
 *
 * [injuries] falta quando a pergunta **não foi respondida**, e não quando não há lesão: quem marcou
 * "Nenhuma" respondeu, e continuar cobrando-a seria um aviso que ela não teria como resolver.
 */
internal data class MissingStudentData(
    ...
    val count: Int
        get() = listOf(level, goal, availableDays, measures, injuries, healthConsent).count { it }
