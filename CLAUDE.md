...
**Coverage has two floors, and they answer different questions**: the project stays at **≥ 60% of lines** (`koverVerify`, so it fails on your machine before it fails in CI), and a PR's **diff stays at ≥ 80%** — measured over the lines the PR changed, commented on the PR, and failing the `verify` job when it doesn't. That failure comes from a **step of our own** reading the action's `coverage-changed-lines` output: at the pinned revision the action's thresholds only pick the comment's emoji and it exits green regardless, which made the floor decorative until `docs/adr/0022` (don't remove that step believing the action gates). Aggregation covers all six modules, and the report **excludes `@Composable`, Room-generated `*_Impl`, `*PreviewFixtures*`, the design-system tokens, declarative wiring (nav graphs, DI containers, callback holders — `docs/adr/0022`), and Firestore adapters listed by name once no rule is left inside them**: without those cuts the percentage measures documented decisions (there are no UI tests by choice) rather than gaps. That last exclusion is earned, not granted — adding a name to the list means showing, in the PR, **where the rule that left the class now lives and which test asserts it**, which is exactly why `LinkDocument`, `LinkRequest` and `InviteCodeDocument` exist as separate testable objects. The rest of the Firebase adapter layer stays in the denominator, `FirestoreStudentRepository` above all: it still holds the health-data consent gate, which is where the largest real gap is. 75% is the declared target, one work item away: the three Firestore repositories are the 190 lines that get there, and that same work empties the exclusion list. The PR comment comes from `madrapps/jacoco-report` pinned **by SHA, not by tag** — the released v1.8.0 gates on whole changed *files*, which would fail a one-line PR touching an old untested file. See `docs/adr/0018` and `docs/adr/0021`.
