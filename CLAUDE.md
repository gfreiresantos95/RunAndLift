...
- `:feature:auth` — welcome, sign in, sign up, password recovery, role selection. Exposes only `navigation.AuthRoutes`, `navigation.authGraph()` and `completeprofile.ProfileCompletion`/`MissingProfileData` (which `:app` needs at launch); everything else is `internal`. Repositories arrive as parameters so the feature never depends on `:app`. The graph **starts on the welcome screen**, where the role is picked before authenticating; the choice travels as a nav argument and is written by sign-up, so nobody is asked twice. Sign-in *reads* the role and never writes it. See `docs/adr/0010`.
  ...
  - **`LinkRepository` is the one repository both role graphs use**: `links` and `inviteCodes`, the collections everything else waits on. It is the only repository that **queries a collection** (`whereEqualTo` on one of the two ids — which is not performance, it's what the rule can authorize) and the only one whose reads go to the **server**: what it reads changes when *the other person* acts, so cache-first would show yesterday's list to whoever opened the screen to see if anyone joined. Cost is one read per link, capped at `LIMIT`; `trainerDashboards` is what replaces the scan later. The link's id is `{trainerId}_{studentId}` (`LinkDocument.id`), `LinkStatus.stored` writes **lowercase** because that is the literal the rules compare, and the decision between creating, reopening and refusing lives in `LinkRequest` — outside the adapter, so a plain JVM test reaches it instead of a fake re-implementing it. **Both names are copied into the link document**: `users/{uid}` is readable only by its owner, so without the copy a trainer's roster would be a list of ids. See `docs/adr/0020`.
- `:feature:student` / `:feature:trainer` — the tabs of each role: home, workouts, menu, plus the trainer's students. **One module per role, not one shared module**, so a student screen cannot import a trainer route by mistake: it isn't on the classpath. Each exposes only its `navigation.*Routes` and `navigation.*Graph()`; repositories arrive as parameters, and `:app` supplies `onSignedOut`/`onSwitchRole` because a feature knows neither the auth route nor the other role. The three tabs are **sibling routes inside the role graph**, never a nested `NavHost` — a tab is not a flow. The role switcher lives in the menu now, still only for accounts that have both roles. See `docs/adr/0016`.
  ...
  - **The student's home names the real trainer**, from the link plus `trainerProfiles/{uid}`: the name travels inside the link document (`users/{uid}` is owner-only), the CREF comes from the profile the Firestore rule opens to anyone with a link, and "aluno desde" comes from `Link.createdAt` — written since day one with nobody reading it, which is why the answer already existed instead of starting to be collected. Only **active or paused** links count as "who trains you": a pending request would have the line assert something the other person hasn't agreed to yet. A CREF that fails to load doesn't erase the trainer from the screen, and a failed link read shows no trainer at all — naming one by guess is worse than naming none. The dashboard numbers next to it are still `StudentDashboard.SAMPLE`, and stay so until `WorkoutRepository` exists.
  ...
  - **The trainer has the same four things, with his questions**: a seven-step onboarding, a profile editor, "Meus dados", and the home banner. What differs is not the shape but two facts about the trainer's document. First, `trainerProfiles/{uid}` **already exists when the flow starts** — sign-up wrote the CREF into it — so "onboarding happened" is a server stamp (`onboardingCompletedAt`, read as `TrainerProfile.onboarded`), never the document's existence; using existence would send every trainer straight to the home having answered nothing. Second, the gated data isn't sensitive, it's **published**: `ShowcaseConsent` is what puts the profile in front of people searching for a trainer, and it gates bio and capacity the way health consent gates weight and injuries. Withdrawing it writes `enabled = false` and **keeps** version and timestamp — the record that publishing was authorized while it lasted — and `showcase.enabled` is the field the Firestore rule reads, so renaming it takes every profile offline without touching a rule. See `docs/adr/0019`.
  - The trainer's questions mirror the student's where they must: the **first five `TrainerSpecialty` values are word-for-word the five `TrainingGoal` values**, which is what lets a search match both sides without a translation table. Specialties are multi-select (the student's goal is single: he has one main goal, a trainer covers several frontiers), and there is no "híbrido" service mode — that is in-person and online ticked together.
  - Empty sets mean **"not answered yet"** in the trainer form, unlike the student's injuries where empty is the answer "none". `toDetails(answered = …)` carries that: a marked set is always written, an empty one only from the edit screen, where the question is on screen and clearing it is a decision.
  - The write mapping lives in `TrainerDocument`, apart from the SDK calls, so the showcase gate can be asserted by a plain JVM test — `TrainerDocumentTest` is the first tested rule in the Firebase adapter layer, and the pattern the other three repositories can follow.
  - **The link is the trainer's fourth tab and the student's menu row**, and that asymmetry is the roles': for the trainer the roster is the product's central screen (Início · Alunos · Treinos · Menu); the student has *a* trainer, not a roster. The trainer's screen groups pending requests above current students above ended ones — a new request lost among thirty known names is a request that waits days — and that grouping lives in `StudentsUiState`, where a test reaches it. **A failed read is never drawn as an empty list**: "you have no students yet" told to a trainer with thirty of them is the worst sentence the screen can produce.
  - **Picking an exercise from the catalog opens its prescription immediately.** Choosing used to drop the exercise into the day with the default numbers and stop there, so the trainer read back "3 × 8-12" he never prescribed and had to tap every row again to say sets, reps, load and rest. `ProgramDraftController.onAddExercise` returns **where the exercise landed** (`null` when the day no longer exists) precisely so the day editor can navigate there — recomputing "size + 1" outside would be silently wrong in exactly that case.
  - **The trainer can read back what a student received**, from the roster's "Ver treino" — the assignment used to be a write nobody could re-open. It reads `assignmentOf(trainerId, studentId)`, which is a **query with both filters and not a `get()` on `{trainerId}_{studentId}`**: the `assignments` rule dereferences `resource.data`, and a rule that errors while evaluating *denies*, so asking for a student who has received nothing would come back as permission denied instead of "doesn't exist" — the most common case of all, the just-accepted student, would read as a network failure. The screen shows the **frozen copy** and not the mould in `programs`, because the question is what the student is holding, not what the mould says today. Days are open sections rather than rows that open a screen: nothing is editable here, the whole document is already in memory, and folding it would cost three screens to answer one question.
  - **Entering a code is two steps: find the trainer, then confirm the name.** They cost the same single read, and the second exists because asking for a link authorizes someone to read weight, height and injuries. Ending is available to the student alone — an arrangement only the other party can end is a subscription, not an arrangement (LGPD art. 18). The invite code is the trainer's, one at a time, reusable, and stored in `trainerProfiles/{uid}.inviteCode` so finding your own code never means listing everyone's.
  - Both modules duplicate `MainDispatcherRule` and the repository fakes in their test source sets, because test source sets are not shared between modules. Thirty repeated lines still cost less than a `:test-fixtures` module; the trigger to extract them is the third module that needs them. The same now goes for the day-of-week labels, the account-form ruler — **not an identical copy: the phone is required for the trainer and optional for the student**, as in sign-up — and the location-picker result keys, where the third copy showed the trigger was aimed wrong: the only home for that glue would be a module shared between features, which is the dependency the architecture refuses.
...
Navigation is **three sibling graphs**, never one graph with role conditionals — a trainer screen must not be reachable from a student's back stack. `MainViewModel` resolves the start destination *before* the NavHost is composed, so the app never opens on the wrong screen for a frame; it has **five outcomes in order** — no session, no role, half-finished sign-up, the role's onboarding, then the home — and the onboarding step asks a different question per role (`onboardingRouteFor`). See `docs/adr/0009` and `docs/adr/0019`. MVVM is the pattern: state out as a read-only `StateFlow`, mutation confined to the ViewModel, no `Context` or UI types in it. Rationale and revisit triggers for the structure, the DI choice, and the deferral of convention plugins are in `docs/adr/0003`.
...
**Coverage has two floors, and they answer different questions**: the project stays at **≥ 60% of lines** (`koverVerify`, so it fails on your machine before it fails in CI), and a PR's **diff stays at ≥ 80%** — measured over the lines the PR changed, commented on the PR, and failing the `verify` job when it doesn't. That failure comes from a **step of our own** reading the action's `coverage-changed-lines` output: at the pinned revision the action's thresholds only pick the comment's emoji and it exits green regardless, which made the floor decorative until `docs/adr/0022` (don't remove that step believing the action gates). Aggregation covers all six modules, and the report **excludes `@Composable`, Room-generated `*_Impl`, `*PreviewFixtures*`, the design-system tokens, declarative wiring (nav graphs, DI containers, callback holders — `docs/adr/0022`), and Firestore adapters listed by name once no rule is left inside them**: without those cuts the percentage measures documented decisions (there are no UI tests by choice) rather than gaps. That last exclusion is earned, not granted — adding a name to the list means showing, in the PR, **where the rule that left the class now lives and which test asserts it**. That is why `LinkDocument`, `LinkRequest`, `InviteCodeDocument`, `StudentDocument`, `UserDocument`, `TrainerDocument`, `ExerciseDocument`, `AuthFailureMapping`, `ProviderName` and `IbgePayload` exist as separate testable objects: **every Firebase adapter has now been drained, and the health-data consent gate — the largest declared gap — is asserted by `StudentDocumentTest`**. The measurement went from 63,4% to **93,8%**, which leaves the 60% floor saying nothing; raising it is a call for the repository owner, not a side effect of the PR that wrote the tests. The PR comment comes from `madrapps/jacoco-report` pinned **by SHA, not by tag** — the released v1.8.0 gates on whole changed *files*, which would fail a one-line PR touching an old untested file. See `docs/adr/0018` and `docs/adr/0021`.
...
Two things the first client of a collection tends to discover, both now fixed in the rules and worth remembering when writing new ones: **`create`, `update` and `delete` want separate declarations** — `request.resource` is null on delete and `resource` is null on create, and a rule that errors while evaluating *denies*, so one line for the three silently blocks half of them; and **a rule that dereferences `resource.data` denies a `get()` on a missing document** rather than answering "doesn't exist", which is why `requestLink` takes the caller's known link instead of looking it up.
...
blank screen, which reads as broken rather than as loading. `AppLoadingState` waits half a second
before showing anything, because a warm-cache read finishes sooner and a flashing spinner reads as
instability; `NO_LOADING_DELAY` inverts that and is **only** for a screen that guarantees the
minimum window on its own side. The catalog is the one that does: every search and every chip holds
`recomputing` for 300 ms in the ViewModel, because filtering in memory finishes before the next
frame and a list that just shrinks reads as a defect rather than as an answer to the tap. Hold that
window in the ViewModel (a `delay` inside a composable is a rule no test reaches), cancel the
previous one on each new tap, and keep the search box and the chips on screen while the list is
being redone — replacing them along with the list takes away exactly what undoes a filter that
closed too far.
...
`AppScreenScaffold` (title + back arrow), `AppTabScaffold` (bottom bar) and `AppListScreenScaffold`
(title + back arrow over a **lazy** list) are the three screen frames; all three wire the top bar's
`pinnedScrollBehavior` internally, which is what keeps a screen from ever touching that experimental
Material API. Scrolling content goes in `AppScreenColumn`, which applies the 600 dp
`Dimens.ContentMaxWidth` — the line that stops the app from being a stretched phone on a tablet or
foldable. `AppScreenLazyColumn` is its lazy sibling and applies the same limit to the whole list
rather than to each item, so no screen has to remember it per `item {}`. **The choice between them
is not taste**: a long, homogeneous list whose size the screen's author doesn't decide goes lazy
(the exercise catalog is hundreds of rows, and in a plain `Column` every filter toggle recomposed
all of them to show the twenty that fit — which is what the screen's flicker was); a form, a record
sheet or a dashboard stays a plain column. Lazy items of data always declare a `key`, or Compose
reuses rows by position. `AppMotion` owns every duration and easing, including the navigation
transitions the root `NavHost` applies; a screen never writes `tween(300)` of its own.
